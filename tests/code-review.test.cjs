  ...
  describe('#3661: configurable code-review hook point (test matrix Section G)', () => {
    ...
    test('G5: tier3PrefersLastReviewCommitOverPhaseStartDerivation', () => {
      ...
      const tier3Block = codeBlocks.find(block =>
        block.includes('DIFF_BASE=""') && block.includes('PHASE_COMMITS'));
      ...
      const phaseCommitsElifIdx = tier3Block.indexOf('elif [ -n "$PHASE_COMMITS" ]; then');
      assert.ok(lastReviewIdx !== -1, 'Tier 3 DIFF_BASE must check LAST_REVIEW_COMMIT');
      assert.ok(phaseCommitsElifIdx !== -1, 'Tier 3 DIFF_BASE must fall back to PHASE_COMMITS via elif (pre-existing derivation unchanged)');
      assert.ok(lastReviewIdx < phaseCommitsElifIdx,
        'LAST_REVIEW_COMMIT must be checked BEFORE the PHASE_COMMITS fallback, so a prior review narrows the diff base');
